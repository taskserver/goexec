...
func TestExec_ContextCancelledDuringExecution(t *testing.T) {
	...
	task := ExecTask{Command: "sleep", Args: []string{"10"}}
	res, err := task.Execute(ctx)
	if err == nil {
		t.Fatalf("want error, but got nil")
	}
...
func TestExec_WithTimeout(t *testing.T) {
	ctx := context.Background()
	task := ExecTask{Command: "sleep", Args: []string{"10"}, Timeout: time.Millisecond * 100}
	res, err := task.Execute(ctx)
	if err == nil {
		t.Fatalf("want error, but got nil")
	}
	...
	if res.Timedout != true {
		t.Fatalf("want timedout, but got false")
	}
...
func TestExec_LogToFiles(t *testing.T) {
	ctx := context.Background()

	// create the output and error files
	outFile, err := os.Create("/tmp/goexec_test_out")
	if err != nil {
		t.Fatalf(err.Error())
	}
	defer outFile.Close()

	errFile, err := os.Create("/tmp/goexec_test_err")
	if err != nil {
		t.Fatalf(err.Error())
	}
	defer errFile.Close()

	task := ExecTask{Command: "echo", Args: []string{"hello"}, OutputFile: outFile, ErrorFile: errFile}
	res, err := task.Execute(ctx)
	if err != nil {
		t.Fatalf(err.Error())
	}
	...
	if res.Stderr != "" {
		t.Fatalf("want empty string Stderr, got %q", res.Stderr)
	}
	...
	if _, err := os.Stat("/tmp/goexec_test_out"); err != nil {
		t.Fatalf("want file to exist, but got %v", err)
	}

	if _, err := os.Stat("/tmp/goexec_test_err"); err != nil {
		t.Fatalf("want file to exist, but got %v", err)
	}
}
